...
int busca(Tree* a, int num){
  if(vazia(a))
    return 0;
  else
    return a->element==num||busca(a->left,num)||busca(a->right,num);
}
...
Tree* insertBalanced(Tree* node, int key)
{
...
int checkHeight(Tree* a){ //tava get antes
  if(a==NULL){
    return -1;
  } else {
    int he = checkHeight(a->left); //tava get antes
    int hd = checkHeight(a->right); //tava get antes
